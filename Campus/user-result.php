...
$sql='SELECT * FROM articles INNER JOIN members on articles.user_id = members.id WHERE members.name=:name';

$statement = $db->prepare($sql);
$statement->execute(['name'=> $membername]);
...
foreach ($statement->fetchAll(PDO::FETCH_ASSOC) as $article ) {
  ...
  // $members= $statement->fetch(PDO::FETCH_ASSOC);
    $articles[]= ['pictures' => $article['pictures'], 'name' => $membername];
}
...
<div class="container">
  ...
  <!-- 検索結果 -->
  <h4><?php echo $article['name'];?> さんの写真</h4>
